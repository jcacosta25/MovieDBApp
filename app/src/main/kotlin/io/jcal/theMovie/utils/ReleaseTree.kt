...
class ReleaseTree : Timber.Tree() {
	...
	override fun log(priority: Int, tag: String?, message: String, t: Throwable?) {
		when (priority) {
			Log.DEBUG -> if (isLoggable(tag, priority)) {
			    tag?.let { Timber.tag(it).d(t, message) }
				?: this.d(t, message)
			}
			Log.VERBOSE -> if (isLoggable(tag, priority)) {
			    tag?.let { Timber.tag(it).v(t, message) }
				?: this.v(t, message)
			}
			Log.INFO -> if (isLoggable(tag, priority)) {
			    tag?.let { Timber.tag(it).i(t, message) }
				?: this.i(t, message)
			}
